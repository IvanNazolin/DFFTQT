...
// 1.Расчтет алгоритма БПФ или ДПФ в фрагменте исходного ВА сигнала
// 2. Расчет спектральных параметров фрагмента исходного ВА сигнала
// *а -  массив значений сигнала силы
// *b -  массив значений сигнала реакции
// FFT - структура параметров ДБПФ
// Type_H -  ви рассчитываемой характеристки: 1 - H1(f), 2 - H2(f)
// *outCount - число значений сигнала *а, *b - на входе, число возвращаемых расчетных значений АЧХ - на выходе
...
// -------------- 
	...
	// Определение и выделение динамической памяти для дублирования значений сигнала силы 
	double* f = (double*)malloc(sizeof(double) * (*outCount));
	if (!f)
	{
		*outCount = 2;	ret[0] = 1.0;	ret[1] = __LINE__;	return ret;
	};

	// Определение и выделение динамической памяти для дублирования значений сигнала реакции 
	double* x = (double*)malloc(sizeof(double) * (*outCount));
	if (!x)
	{
		free(f); *outCount = 2;	ret[0] = 1.0;	ret[1] = __LINE__;	return ret;
	};

	// Дублирование значений временного ряда *а в массив *f, ряда *b  в массив *x
	for (i = 0; i < *outCount; i++)
	{
		f[i] = a[i];	x[i] = b[i];
	};
 
	// 1. Расчет комплексного спектра сигнала силы с использованием алгоритма БПФ, ДПФ
	...
	if (!(Re = (double*)malloc(sizeof(double) * ParFFT.npt)))
	{
		free(f); *outCount = 2;	ret[0] = 1.0;	ret[1] = __LINE__;	return ret;
	};

	// Выделение динамической памяти для массива Im результатов расчета ПФ
	if (!(Im = (double*)malloc(sizeof(double) * ParFFT.npt)))
	{
		free(f); *outCount = 2;	ret[0] = 1.0;	ret[1] = __LINE__;	return ret;
	}
	...
	for (point = 0, i = 1; i <= ParFFT.nBlocks; i++, point += ParFFT.Offset)
	{
		...
		{
			Re[n] = a[point_n];	Im[n] = 0.0;
		}

		// Расчет алгоритма БПФ или БПФ для текущего блока
		if (!Fft_transform(Re, Im, ParFFT.npt))
		{
			free(Re); free(Im); free(f); 
			*outCount = 2;	ret[0] = 1.0;	ret[1] = __LINE__;	return ret;
		}

		// Расчет среднего спектра типа Type_spec по блокам
		Estimate_Sym_Spec(f, Re, Im, ParFFT.np, ParFFT.dF, ParFFT.nBlocks, ParFFT.TypeSpec);
	};




	//Определение и выделение динамической памяти под массив результата расчета АЧХ
	double* H = (double*)malloc(sizeof(double) * (ParFFT.np));
	if (!H)
	{
		free(f); free(x); *outCount = 2;	ret[0] = 1; ret[1] = __LINE__;	return ret;
	};
	// расчет спектра по алгоритму ДБПФ
	if ((Kod_Program = DFTAnalysis(f, H, ParFFT, &Kod_LINE)) != 0)
	{
		free(f);	free(H);		*outCount = 2;		ret[0] = Kod_Program;	ret[1] = Kod_LINE;	return ret;
	};
